...
- A delivery token naming another tenant returning **302, with the asset served**, once the tenant check is
  removed.
- In one historical 410-test browser-suite diagnostic, **4, 3 and 1** tests failed across three full
  runs, with no test failing in more than one run. This is evidence about that run's flakiness, not the
  current suite size.

## A note on what these posts admit

Several posts describe things we got wrong. The storage scrub still cannot classify every unreadable
object, and the asset page does not yet fetch new windows beyond its first 60 assets. That is deliberate.
A series about a storage system that only described its successes would make exactly the claim the
fifth post argues against: that a system's silence about its own faults is evidence there are none.
