...
/*
	The `/assets` cases live in `browse.e2e.ts` now. This page no longer carries sample data — it reads a
	real API — so its grid, keyboard and live-region cases need the route mocking that file sets up. Split
	rather than moved wholesale: everything above is about the *shell* and holds regardless of what any page
	fetches.
*/
