	...
	<div class="flex flex-wrap items-end justify-between gap-3">
		<div>
			...
			<p class="mt-1 max-w-2xl text-sm text-muted">
				These fields are what every form, filter and search in this tenant is built from. The count
				beside each one is how many assets already carry a value for it.
			</p>
		</div>
		<button
			type="button"
	...
	{#if adding}
		<form class="flex flex-wrap items-end gap-3 rounded-md bg-surface p-4" onsubmit={add}>
			...
			<p class="w-full text-xs text-muted">
				A key is permanent: it is the name every stored value sits under, so renaming it later means
				adding a new field and moving the data. Lower-case letters, digits and underscores.
			</p>
		</form>
	{/if}
